...
contract('RealEstate', function (accounts) {
    ...
    it("가나슈 두번째 계정으로 매물 아이디 0번 매입 후 이벤트 생성 및 매입자 정보와 buyers 배열 테스트", function () {
        return RealEstate.deployed().then(function (instance) {
            realEstateInstance = instance;
            return realEstateInstance.buyRealEstate(0, "vega2k", 20, { from: accounts[1], value: web3.toWei(1.50, "ether") });
            ...
            //Buyer의 name '0x76656761326b0000000000000000000000000000000000000000000000000000'
            assert.equal(web3.toAscii(buyerInfo[1]).replace(/\0/g, ''), "vega2k", "매입자의 이름이 vega2k가 아닙니다.");
            assert.equal(buyerInfo[2], 20, "매입자의 나이가 20살이 아닙니다.");
